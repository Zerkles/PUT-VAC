...
def process_image(size, img, highpass=False):
    ...
    img_arr = cv2.resize(img_arr, (0, 0), fx=resampling_factor[1], fy=resampling_factor[0],
...
def gen_sound_from_image(img, duration=5.0, sample_rate=44100.0, intensity_factor=1, min_freq=0,
    ...
    if stepping_spectrum < 0:
        stepping_spectrum = -stepping_spectrum

    img_mat = process_image(size=(stepping_spectrum, max_frame), img=img, highpass=highpass)
    if invert:
        img_mat = 1 - img_mat
    # end if
...
class Vac:
    ...
    def feed_image(self, img):
        ...
        self.proc_pool.apply_async(func=gen_sound_from_image, args=args, callback=self.finish_callback)
        '''
        buf = gen_sound_from_image(
            ...
            highpass=highpass,
            img_dict_val=self.img_dict_val
        )
        self.finish_callback(buf)
        '''
        return
